...
// CORS - allowlist from env + *.dreamexdatalab.com by default
const ALLOWED_ORIGINS = (process.env.CORS_ALLOWED_ORIGINS || 'https://dreamexdatalab.com,https://www.dreamexdatalab.com')
    .split(',')
    .map(s => s.trim())
    .filter(Boolean);
...
app.use((req, res, next) => {
    try {
        ...
        if (origin) {
            ...
            const isAllowed = ALLOWED_ORIGINS.includes(origin) || host === 'dreamexdatalab.com' || host.endsWith('.dreamexdatalab.com');
            if (isAllowed) {
...
// ============================================================================
// CONFIGURATION - utilise variables d'environnement si présentes
// ============================================================================

const ORANGE_CONFIG = {
    // Fourni par Orange Money Guinée
    client_id: process.env.CLIENT_ID || process.env.ORANGE_CLIENT_ID || 'ZLw7FjAEJ3h97ZcF2k2qG93tq2LDFsFM',
    client_secret: process.env.CLIENT_SECRET || process.env.ORANGE_CLIENT_SECRET || 'V6mpOxb7TFWQws2YKHE6GJyuA5j2RBYoamcoFGH36gdO',
    // Merchant key fourni par Orange Money (WebPay GN)
    merchant_key: process.env.OM_MERCHANT_KEY || process.env.ORANGE_MERCHANT_KEY || '427ab675',
    ...
    authorization: process.env.OM_AUTHORIZATION || process.env.ORANGE_AUTHORIZATION || 'Basic Wkx3N0ZqQUVKM2g5N1pjRjJrMnFHOTN0cTJMREZzRk06VjZtcE94YjdURldRd3MyWUtIRTZHSnl1QTVqMlJCWW9hbWNvRkdIMzZnZE8='
};
