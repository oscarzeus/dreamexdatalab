...
## Prerequisites
- Domains: `dreamexdatalab.com` for the frontend and `api.dreamexdatalab.com` for the backend API, both pointing to your server
...
Create Nginx configuration for the API subdomain:
```bash
sudo nano /etc/nginx/sites-available/api.dreamexdatalab.com
```
...
server {
    listen 80;
    server_name api.dreamexdatalab.com;
...
server {
    ...
    location /payment/success.html {
        proxy_pass http://localhost:8080/payment/success.html;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location /payment/cancel.html {
        proxy_pass http://localhost:8080/payment/cancel.html;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location /api/payments/webhook {
        proxy_pass http://localhost:8080/api/payments/webhook;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
...
### Enable the site:
```bash
sudo ln -s /etc/nginx/sites-available/api.dreamexdatalab.com /etc/nginx/sites-enabled/
sudo nginx -t
...
Update your production `.env` (note the separate frontend and API domains):
```bash
cd /var/www/orange-money-production
sudo nano .env
```
